...
def greedyBFSUtil(G, v, visited, final_path, dest, goal): 
	...
	if v == dest:
		...
		pq_list = []
		pq,size = getPriorityQueue(list(G[v]))
		for i in range(size):
...
def greedyBFS(G, source, dest, heuristics, pos): 
	...
	for var in final_path:
		if prev != -1:
			curr = var			
			nx.draw_networkx_edges(G, pos, edgelist = [(prev,curr)], width = 2.5, alpha = 0.8, edge_color = 'red')
			prev = curr
		else:
			prev = var
	return
...
def getHeuristics(G):
	heuristics = {}
	f = open('gbfsheuristics.txt')
	for i in G.nodes():
		node_heuristic_val = f.readline().split()
		if len(node_heuristic_val)==0:
			break
